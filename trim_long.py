...
class FixedWingDynamics:
    ...
    def compute_forces(self, x, u, wind_body):
        ...
        # Calculate relative wind speed (body frame)
        u_rel = u_b - wind_body[0]
        v_rel = v_b - wind_body[1]
        w_rel = w_b - wind_body[2]
        
        # Calculate angle of attack and sideslip angle using body-frame relative velocity
        ...
        # Lift force (airflow frame)
        Lift = q_dynamic *  CL
        
        # Side force (airflow frame)
        Y = q_dynamic * CY
        
        # Drag force (airflow frame)
        D = q_dynamic * CD
         
        # Total force in body frame (X,Y,Z)
        ...
        return np.array([F_x, F_y, F_z, L, M, N])  # Lateral force simplified
    ...
    def dynamics(self, t, x, u):
        ...
        # Altitude constraint: force stop descending when Z_down approaches 0
        
        if x[11] >= 0:  # Z_down >= 0 indicates ground contact
           x[11] = 0  # Clamp altitude
           dZ_down = max(0, dX_ned[2])  # Allow ascent only
...
class TrimCalculator:
    ...
    def find_trim(self, gamma=0, t_trim=0.0):
        ...
        def equations(vars):
            ...
            if delta_e_sync < delta_e_sync_min:
                delta_e_sync_constraint = (delta_e_sync - delta_e_sync_min) * 1e6  # Large penalty
            ...
            if delta_e_diff < delta_e_diff_min:
                delta_e_diff_constraint = (delta_e_diff - delta_e_diff_min) * 1e6  # Large penalty
            ...
            # Indirectly solve for V and alpha from u, w
            V = np.sqrt(u_rel**2 + v_rel**2 + w_rel**2)
            alpha = np.arctan2(w_rel, u_rel) if V > 1e-3 else 0
            theta = alpha + gamma
            ...
            Cm_sync = (self.model.Cm0 + self.model.Cm_alpha * np.rad2deg(alpha) + 
                  ...
                  self.model.Cm_delta_e_sync * np.rad2deg(delta_e_sync))  # Pitching moment coefficient, tail synchronized deflection
    ...
    def linearize_trim(self, epsilon=1e-6):
        """Linearization based on trim state with small perturbations"""
        if not hasattr(self, 'trim_state'):
            raise RuntimeError("Please run find_trim() first to obtain trim state")
    
        # Extract trim state
        # Longitudinal states
        x0 = np.array([
            self.trim_state['V'],  # V 
            self.trim_state['alpha'],  # α
            0.0,                        # q
            self.trim_state['alpha'],  # θ
            -100.0,                      # H
            ])
        ...
        for i in range(n_controls):
            u_plus = u0.copy()
            u_plus[i] += epsilon
            # Compute state derivatives before and after perturbation
            dx0 = f(x0, u0)
            dx_plus = f(x0, u_plus)
    ...
    def dynamics(self, t, x, u):
        ...
        # Position increments
        dX_ned = R @ np.array([u_b, v_b, w_b])
        dX, dY, dH = dX_ned[0], dX_ned[1], -dX_ned[2]
    ...
    def analyze_transfer_function(self, sys, input_name, output_name):
        ...
        """
        Extract and print the transfer function of a state-space model (supports SISO only)
        :param sys: Control system state-space model (ctrl.ss)
        :param input_names: List of input names (e.g., ['delta_e_sync'])
        :param output_names: List of output names (e.g., ['theta'])
        """
        ...
        C_sub = sys.C[output_index, :].reshape(1, -1)  # Keep only the row for theta
    ...
    def analyze_transfer_functions(self, sys, input_names, output_names):
        ...
        # Analyze pitch channel: delta_e_sync -> theta (pitch angle)
        ...
        plt.title('Pitch: δe_sync → θ', fontsize=20)
    ...
    def simulate_perturbation(self, t_end=90.0):
        ...
        # Plot curves (use academic style colors)
        ...
        ax.set_ylabel('State Variation', labelpad=10)
        ...
        plt.savefig('trim_results/5_Longitudinal dynamic response to pitch perturbation.png', dpi=600, bbox_inches='tight')
        plt.show()
